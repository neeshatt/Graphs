...
document.getElementById("run-js").addEventListener("click", function() {
    ...
    document.getElementById("outputbox").value = ""; // Clear the level order traversal output
    inputStep = 0;
    nodes = {};
    currentNode = null;
    parentStack = [];
...
function levelOrderTraversal(rootNode) {
    ...
    console.log("Finished level order traversal, result:", result);
    return result;
}
...
function finishTreeConstruction() {
    ...
    let traversalResult = levelOrderTraversal(rootNode);
    let traversalString = traversalResult.map(level => level.join(' ')).join('\n');
    
    console.log("Traversal result:", traversalString);
    document.getElementById("outputbox").value = `Your Level Order Route Is:\n\n${traversalString}`;

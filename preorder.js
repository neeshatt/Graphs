...
document.getElementById("run-js").addEventListener("click", function () {
    ...
    document.getElementById("outputbox").value = ""; // Clear the preorder traversal output
    inputStep = 0;
    nodes = {};
    currentNode = null;
    parentStack = [];
...
function preorderTraversal(rootNode) {
    ...
    console.log("Starting preorder traversal from root:", rootNode);
    traverse(rootNode);
    console.log("Finished preorder traversal, result:", result);
    return result;
}
...
function finishTreeConstruction() {
    ...
    let traversalResult = preorderTraversal(rootNode);
    let traversalString = traversalResult.join(' -> ');

    console.log("Traversal result:", traversalString);
    document.getElementById("outputbox").value = `Your Pre Order Route Is: ${traversalString}`;
